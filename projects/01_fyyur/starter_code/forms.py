...
class VenueForm(Form):
    ...
    state = SelectField(
        'state', validators=[DataRequired(), AnyOf([x[0] for x in state_list]),
        choices=state_list
    )
    address = StringField(
    ...
    phone = StringField(
        'phone', validators=[Regexp(
            ...
            message='Please include a valid phone number in xxx-xxx-xxxx format'
        ), DataRequired()]
    )
    ...
    genres = SelectMultipleField(
        'genres', 
        choices=genre_list,
        validators=[DataRequired(), AnyOf(genre_list)],
    )
...
class ArtistForm(Form):
    ...
    genres = SelectMultipleField(
        'genres', 
        choices=genre_list,
        validators=[DataRequired(), AnyOf([x[0] for x in genre_list])],
    )
    facebook_link = StringField(
        'facebook_link', validators=[URL()]
    )
